"""MiniMax H3 VAEs (video + audio) - self-contained torch port.

Ported from ``comfy/ldm/minimax/vae.py`` and ``comfy/ldm/minimax/audio_vae.py``
(ComfyUI PR #15224) with every ComfyUI dependency replaced:

* ``ops.Conv3d/Conv1d/Linear/LayerNorm`` -> ``nn.*``
* ``ops.RMSNorm`` + ``comfy.rmsnorm.rms_norm`` -> local ``_RMSNorm`` / ``_rms_norm``
* ``comfy.quant_ops.ck.apply_rope_split_half`` -> eager ``_apply_rope_split_half``
* ``optimized_attention`` -> ``F.scaled_dot_product_attention``
* ``comfy.ops.cast_bias_weight`` offload path removed (plain F.linear)

The video VAE tiles internally (256 px spatial, 17-frame temporal chunks with
overlap) and normalises latents with the checkpoint's ``latents_mean/std``;
the audio VAE is a DAC-style encoder + BigVGAN decoder at 32 kHz.
"""
...
def _apply_rope_split_half(xq, xk, table):
    """Split-half RoPE (first-half/second-half pairing), eager port of
    comfy_kitchen's ``apply_rope_split_half(xq, xk, freqs_cis)``.
    ``table``: [..., pairs, 2, 2] rotation matrices [[c, -s], [s, c]]."""
    n_pairs = table.shape[-3]
    c = table[..., 0, 0]
    s = table[..., 1, 0]
...
class CausalConv3d(_Conv3dBase):
...
class Attention(nn.Module):
    ...
    def forward(self, x, rotary_pos_emb=None):
        ...
        if rotary_pos_emb is not None:
            ...
            query[..., :rot], key[..., :rot] = _apply_rope_split_half(
...
class TransformerBlock(nn.Module):
    def __init__(self, heads, dim_head, bias=True, eps=1e-5):
        super().__init__()
        dim = heads * dim_head
        self.norm1 = _RMSNorm(dim, elementwise_affine=True, eps=eps)
        self.attn = Attention(heads=heads, dim_head=dim_head, bias=bias, eps=eps)
        self.scale1 = nn.Parameter(torch.empty(dim))
        self.norm2 = _RMSNorm(dim, elementwise_affine=True, eps=eps)
        self.ff = FeedForward(dim=dim, bias=bias)
        self.scale2 = nn.Parameter(torch.empty(dim))

    def forward(self, x, rotary_pos_emb=None):
        x = x.addcmul_(self.attn(_rms_norm(x, self.norm1.weight, self.norm1.eps), rotary_pos_emb), self.scale1)
        return x.addcmul_(self.ff(_rms_norm(x, self.norm2.weight, self.norm2.eps)), self.scale2)
...
class CausalAttention(nn.Module):
    ...
    def forward(self, x):
        B, N, C = x.shape
        qkv = F.linear(x, self.qkv.weight, bias=torch.cat((self.q_bias, self.zero_k_bias, self.v_bias)))
...
class VAEPack:
    """Video + audio VAE pair with encode/decode + explicit unload."""
    ...
    def encode_video(self, x: torch.Tensor) -> torch.Tensor:
        ...
        device = self.video_vae.quant_conv.weight.device
        return self.video_vae.encode(x.to(device))
    ...
    def encode_audio(self, wav: torch.Tensor) -> torch.Tensor:
        ...
        device = self.audio_vae.mean_proj.weight.device
        return self.audio_vae.encode(wav.to(device))

    def decode_audio(self, z: torch.Tensor) -> torch.Tensor:
        assert self.audio_vae is not None
        device = self.audio_vae.dec_in_proj.weight.device
        return self.audio_vae.decode(z.to(device))
        ...
        for vae in (self.video_vae, self.audio_vae):
            ...
            for mod in vae.modules():
                for name, p in list(mod._parameters.items()):
                    if p is not None:
                        mod._parameters[name] = nn.Parameter(torch.empty(0), requires_grad=False)
...
def _detect_vae_prefix(reader: BlockReader) -> str:
...
def load_vae_pack(video_path: str, audio_path: str, device=None,
                  pin: bool = True) -> VAEPack:
    """Load (and cache) the video/audio VAE pair from safetensors files."""
    cache_key = f"{video_path}|{audio_path}"
