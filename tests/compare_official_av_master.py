"""Compare latest ComfyUI-master MiniMax H3 AV forward with local master mode."""

import os
import sys

_NODE_ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
_MASTER = r"C:\Users\Administrator\Downloads\ComfyUI-master\ComfyUI-master"
...
from h3rt.models.model import time_shift_sigma
...
official_out = official(
    [video, audio_scaled], timestep, text,
    transformer_options={
...
print("video rel",
      (ours_out[0].float() - official_out[0].float()).abs().max().item()
      / max(1e-6, official_out[0].float().abs().max().item()))
print("audio_y rel",
      (ours_audio_y.float() - official_out[1].float()).abs().max().item()
